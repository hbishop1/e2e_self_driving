...
class Original_PilotNet(nn.Module):
    def __init__(self,stereo=True,dropout_conv=0.5,dropout_fc=0.5):

        super(Original_PilotNet, self).__init__()

        #self.register_buffer('mse', torch.tensor([0]))
        ...
        self.feature_extract_layers.append(nn.Dropout(dropout_conv))
        ...
        self.regression_layers.append(nn.Dropout(dropout_fc))
